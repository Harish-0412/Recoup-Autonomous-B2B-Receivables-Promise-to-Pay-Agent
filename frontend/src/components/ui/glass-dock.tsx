...
type DockIcon = React.ComponentType<{ className?: string }> | string;
...
if (typeof window !== "undefined") {
    try {
        ...
        import("gsap/MorphSVGPlugin").then((plugin) => {
            ...
            // MorphSVGPlugin is a Club GSAP premium plugin; falls back smoothly if not present
        });
    } catch (e) {
        // MorphSVGPlugin fallback
    }
}
...
const MorphingIcon = ({ type, isActive, onClick, onMouseEnter }: { type: string, isActive: boolean, onClick: () => void, onMouseEnter?: () => void }) => {
        ...
        gsap.to(button, {
            ...
            onComplete: () => {
                try {
                    ...
                    gsap.to(button, { scale: 1.15, yoyo: true, repeat: 1, duration: 0.2 });
                }
            },
        });
    };
    ...
    const baseBtnCls = "relative flex items-center justify-center w-8 h-8 rounded-lg text-zinc-500 hover:text-zinc-950 dark:text-zinc-400 dark:hover:text-white transition-colors";

    if (type === 'home') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("home", baseBtnCls, isActive ? "active text-zinc-950 dark:text-white" : "")}>
    ...
    if (type === 'blog') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("blog", baseBtnCls, isActive ? "active text-zinc-950 dark:text-white" : "")}>
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
    ...
    if (type === 'marker') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("marker", baseBtnCls, isActive ? "active text-zinc-950 dark:text-white" : "")}>
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
    ...
    if (type === 'email') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("email", baseBtnCls, isActive ? "active text-zinc-950 dark:text-white" : "")}>
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
    ...
    if (type === 'linkedin') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("linkedin", baseBtnCls, isActive ? "active text-zinc-950 dark:text-white" : "")}>
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
    ...
    if (type === 'x') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("x", baseBtnCls, isActive ? "active text-zinc-950 dark:text-white" : "")}>
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current">
    ...
    if (type === 'github') {
        return (
            <button ref={buttonRef} onClick={onClick} onMouseEnter={handleMouseEnter} className={cn("github", baseBtnCls, isActive ? "active text-zinc-950 dark:text-white" : "")}>
                <svg viewBox="0 0 24 24" className="w-5 h-5 fill-current" fillRule="evenodd" clipRule="evenodd">
...
export const GlassDock = React.forwardRef<HTMLDivElement, GlassDockProps>(
    (
        ...
        const getTooltipPosition = (index: number) => index * 48 + 8;
        ...
        return (
            ...
            >
                <div
                    className={cn(
                        "glass-dock relative flex gap-2 sm:gap-3 items-center px-4 py-2.5 rounded-2xl",
                        "border border-zinc-200/80 dark:border-white/15 bg-white/80 dark:bg-black/80 backdrop-blur-2xl shadow-2xl justify-center transition-colors duration-300",
                        dockClassName
                    )}
                    onMouseLeave={() => {
                    ...
                    {items.map((el, index) => {
                        ...
                        const handleClick = () => {
                            if (el.onClick) {
                                ...
                                if (el.href.startsWith("#")) {
                                    const element = document.querySelector(el.href);
                                    if (element) {
                        ...
                        return (
                            <div
                                ...
                                onClick={handleClick}
                                className="relative w-9 h-9 flex items-center justify-center cursor-pointer rounded-xl hover:bg-black/5 dark:hover:bg-white/10 transition-colors"
                                role="button"
                                tabIndex={0}
                                onKeyDown={(e) => {
